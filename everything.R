...
system('ls')
# could use this to make sure all necessary files are present 
...
tissue.alpha.16S <- ggplot(Tissue21.meta) + geom_boxplot(aes(tissue, invsimpson, fill = treatment)) +
...
alpha = 0.1 # Changing the alpha cutoff to 0.1 so that each timepoint can have a table
...
write.table(sigtab.D21, file = './OTUs/feceslumped_OTUs.txt', row.names = TRUE, quote = FALSE, col.names = TRUE, sep = '\t')
...
####### NEED TO WRITE DIFFABUND OTUS for buts ###########
...
p.flow.cecsig <- filter(fin, pvalue < 0.05 & tissue == 'cec') %>% ggplot(aes(x=treatment, y=value, fill=treatment))+geom_boxplot()+
  geom_jitter(shape=21, aes(fill=treatment), size =2, stroke=.75, width = .1)+
  facet_wrap(~tisXatt+pvalue2, scales = 'free')+ scale_fill_brewer(palette="Dark2")+
  labs(x="treatment", y= "Percent Live Single cells") + theme(strip.text = element_text(size = 9))
...
##################### THIS CHUNK IS FOR SELECTING IMPORTANT VERTEXES ###############
# make list of important nodes, vector sequence? then feed this into make_ego_graph
...
############# FILE READ HERE ##################
############# COULD REPLACE THIS WHEN WRITING OTU FILES, STILL NOT SURE I NEED THEM #######
...
############## NEED TO FOLD IN GLYCOSIDE HYDROLASE DATA FROM WORK ############
...
# 
# 
# D21fec.sigs <- ccrepe_to_ggnet(ccrepe.obj = but_16S_D21fec, pcut = 0.05, spearcut = 0.7)
# 
# #D21vfa_otu.sigs <- ccrepe_to_ggnet(ccrepe.obj = vfa_16S_D21fec, pcut = 0.05, spearcut = 0.6)
# 
...
but_16S_cec.sigs <- ccrepe_to_ggnet(ccrepe.obj = but_16S_cec, pcut = 0.05, spearcut = 0.6)
cec_16S.sigs <- ccrepe_to_ggnet(ccrepe.obj = cec_16S, pcut = 0.05, spearcut = 0.6)
cec_but.sigs <- ccrepe_to_ggnet(ccrepe.obj = cec_but, pcut = 0.05, spearcut = 0.6)
vfa_glyc.sigs <- ccrepe_to_ggnet(ccrepe.obj = vfa_glyc, pcut = 0.05, spearcut = 0.6)
vfa_16S.sigs <- ccrepe_to_ggnet(ccrepe.obj = vfa_16S, pcut = 0.05, spearcut = 0.6)
vfa_but.sigs <- ccrepe_to_ggnet(ccrepe.obj = vfa_but, pcut = 0.05, spearcut = 0.6)
vfa_16S.sigs <- ccrepe_to_ggnet(ccrepe.obj = vfa_16S, pcut = 0.05, spearcut = 0.6)
glyc_16S.sigs <- ccrepe_to_ggnet(ccrepe.obj = glyc_16S, pcut = 0.05, spearcut = 0.6)
glyc_but.sigs <- ccrepe_to_ggnet(ccrepe.obj = glyc_but, pcut = 0.05, spearcut = 0.6)
...
TxB <- ccrepe(x = cecbact, y = flowr, min.subj = 4, verbose = TRUE)
bac <- ccrepe(x = cecbact, min.subj = 4, verbose = TRUE, compare.within.x = TRUE)
im <- ccrepe(x = flowr, min.subj = 4, verbose = TRUE)
...
TxB.sigs <- ccrepe_to_ggnet(TxB, spearcut = 0.6)
im.sigs <- ccrepe_to_ggnet(im, spearcut = 0.6)
