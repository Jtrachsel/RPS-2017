...
write.table(otu.sigtab.wean, file = 'feceslumped_OTUs.txt', row.names = TRUE, quote = FALSE, col.names = TRUE, sep = '\t')
...
write.table(dispersion_supp_16S, file = '16S_dispersion_stats.txt', quote = FALSE, row.names = FALSE, sep = '\t')
...
p.16s.disp.time <- ggplot(fecaldisptime, aes(x=day, y=average_dist, color = treatment)) + 
  geom_vline(xintercept = 12)+ geom_point(size = 2.5) +  geom_line(show.legend = FALSE) + ylim(c(.2,.5))+ xlim(c(0,22))+
  ...
  ylab("Avg dist to group mean") + scale_color_brewer(palette = "Dark2") +
  annotate(geom = 'text', x=21, y=.4, label='* p=0.001 *')
p.16s.disp.time
#############FIG 2 16S DISPER #############
...
p.weaning.ord <- ggplot(data=subset(FS2.metanmds, tissue == 'feces' & day %in% c(0,21)), aes(x=MDS1, y = MDS2)) +
  ...
  ggtitle('Feces community similarity', subtitle = 'Day 0 vs Day 21 within group\nPERMANOVA: control: F=10.8, p=0.0001; RPS: F=11.5, p=0.0001') + 
...
p.tissord <- FS2.metanmds %>% filter(day %in% c(21)) %>% ggplot(aes(x=groupX, y=groupY)) +
  ...
  geom_text(data=groupxtiss, aes(x=groupX, y=groupY, label=tissue), show.legend = FALSE)+
...
just_poop <- pwadon[grep('feces', pwadon$pairs),]
...
meta <- meta[-57,]  # why? this was to remove pig 97 from the data but it appears to be gone already
meta[57,]
...
but2 <- but2[-57,]
but2[57,]
write.table(but2, '~/FS2/Everything/but_shared_forcorr.txt', col.names = TRUE, row.names = FALSE, quote = FALSE)
...
write.table(dispersion_supp_all,'dispersion_supp_all.txt' ,sep = '\t', quote = FALSE, row.names = FALSE, col.names = TRUE)
...
p.butfecal.dispersionAVG <- ggplot(but.fecaldisptime, aes(x=day, y=average_dist, color = treatment)) + 
  ...
  #ggtitle(my_title,
  #        subtitle = "Vegan's betadisper(): how much variability is there in a group's community structure?") + 
  ylab("Avg dist to the group mean") + scale_color_brewer(palette = "Dark2")
...
p.but.otu87.fec <- but22 %>% filter(otu == 'Otu067') %>%
  ggplot(aes(x=day, y=value))  +
  #geom_vline(xintercept = 12, colour='black')+
  geom_boxplot(aes(x=day, y=value, group=design, fill=treatment), show.legend = FALSE) +# ggtitle('Relative proportion of OTU87 in feces over time')
  scale_fill_brewer(palette = 'Dark2')+ ylab('proportion of total community')
...
write.table(meta, '~/FS2/Everything//but_meta_forcorr.txt', col.names = TRUE, row.names = FALSE, quote = FALSE)
...
master.results <- filter(master.results, substrate != "4-nitrophenyl sulfate")
...
filter(master.results, pig != 'NTC' & tissue == 'feces' & r.squared > 0.7 & substrate %in% c('a-L-fucopyranoside')) %>%
  ggplot(aes(x=day, y=rate, group=design)) +
  geom_boxplot(aes(fill=treatment)) + 
  scale_fill_brewer(palette = 'Dark2') + facet_wrap(~substrate, scales = 'free') + expand_limits(y=0)

filter(master.results, pig != 'NTC' & tissue == 'feces' & r.squared > 0.7 & substrate %in% c('B-D-xylopyranoside', 'a-D-galactopyranoside', 'A-D-glucopyranoside')) %>%
  ggplot(aes(x=day, y=rate, group=design)) +
  geom_boxplot(aes(fill=treatment)) + 
  scale_fill_brewer(palette = 'Dark2') + facet_wrap(~substrate, scales = 'free') + expand_limits(y=0)
...
write.table(tests, file='Nitro_stats.txt', sep = '\t', quote = FALSE, row.names = FALSE)
...
enrich <- rbind(dif_ab_16S, dif_ab_but)
#colnames(dif_ab_but[,c(7,8,9,10,11,12)]) %in% colnames(dif_ab_16S)
...
p.butnet1 <- ggplot(filtered5, aes(from_id = from_id, to_id = to_id, label=from, color=type)) + 
...
p.qPCR <- filter(qPCR.sum, gene %in% c('DEFB1', 'IL.6', 'MUC2')) %>%
  ...
  scale_color_brewer(palette = 'Dark2') + ylab('Relative expression (log2 fold change)') + xlab('gene')
...
pl <- ggplot(newplot, aes(color=type)) +
  ...
  annotate('point', x=1.027, y=1.08, color="#E76BF3", size=8) + annotate('text', x=1.072, y=1.08 , label='VFA', size=3, color='black') +
  annotate('point', x=1.027, y=1.04, color="#F8766D", size=8) + annotate('text', x=1.072, y=1.04 , label='16S', size=3, color='black') +
  annotate('point', x=1.027, y=1.00, color="#619CFF", size=8) + annotate('text', x=1.072, y=1.00, label='other', size=3, color='black') +
  annotate('point', x=1.027, y=.96, color="#00BF7D", size=8) + annotate('text', x=1.072, y=.96 , label='CD3+', size=3, color='black') +
  annotate('point', x=1.027, y=.92, color="#B79F00", size=8) + annotate('text', x=1.072, y=.92 , label='CD3-', size=3, color='black') +
  ...
  annotate('rect', xmin=1.004, xmax=1.05, ymin=.879, ymax=.881, color="yellow", fill = 'yellow') +
  annotate('rect', xmin=1.026, xmax=1.028, ymin=.818, ymax=.862, color="black", fill = 'black') +
...
fig.2 <- ggdraw() + 
  draw_plot(p.alladon, 0,0,.5,1)+
  draw_plot(p.16s.disp.time, .5,0,.5,1)+
